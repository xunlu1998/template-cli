 ...
 * @zhangbobo
 */
module.exports = class PairDirect extends ModelPair {
  /**
   * @param {*} L 长
   * @param {*} W 宽
   * @param {*} H 高
  ...
  constructor(
    ...
    this.AX = this.getcheck(
      ...
      0,
      this.roundM(
        Math.min((this.W + 2 * this.id) / 3, 5 * this.scale + this.D),
        this.scale,
        0.5
      ),
      "防尘翼间隙"
    ...
    // 上盖参数
    this.CX = this.D;
    this.slot = this.getcheck(
      this.getC(),
      5 * this.scale,
      this.roundM(this.kh / 2),
      "插舌长度"
    );
    ...
    this.ct = this.getcheck(
      0,
      0,
      (Math.min(30, 0.5 * Math.atan((this.kl / 2 - this.CX) / this.slot)) *
        180) /
        Math.PI,
      "角度偏移",
      true
    ...
    this.AXX = this.getcheck(
      this.getAXX(),
      0,
      this.AX + 2 * this.scale,
      "上开口"
    );

    this.S = this.getS();
    this.AS = this.AX;
    this.NX = this.getcheck(
      this.getNX(),
      0,
      this.roundM(this.kw / 5),
      "公锁X坡度"
    );
    this.NY = this.getcheck(
      this.getNY(),
      0,
      this.roundM(this.kw / 5),
      "公锁Y坡度"
    );
    this.AR = this.getcheck(
      this.getAR().init,
      0,
      this.getAR().max,
      this.getAR().name
    ...
    this.A = this.getcheck(
      Math.min(
        this.ceilM(this.kw / 2 + this.slot / 2),
        this.floorM(this.kl / 2)
      ),
  ...
  getNX() {
    let W = this.kw;
    let s =
      W > 8 * this.scale && W <= 30 * this.scale
        ? 2 * this.scale
        : 3 * this.scale;
      ...
      },
      {
        ...
        dlist: () => {
          let x =
            this.slot -
            this.CR +
            this.CR * Math.cos(((90 - this.ct) * Math.PI) / 180);
          return [
        ...
        dlist: () => {
          return [
            ...
            },
            {
              mtd: "Q",
              cx:
                this.x2 - this.AX1 - this.NX - this.getARX1() - this.getARX2(),
            ...
            {
              mtd: "L",
              x: this.x2 - this.AX1 - this.NX,
              y: this.y1 + this.of - this.S - this.NY,
            },
        ...
        dlist: () => {
          return [
            ...
            },
            {
              mtd: "Q",
              cx:
                this.x1 + this.AX1 + this.NX + this.getARX1() + this.getARX2(),
              cy: this.y2 + this.A,
            ...
            {
              mtd: "L",
              x: this.x1 + this.AX1 + this.NX,
              y: this.y2 + this.S + this.NY,
            },
        ...
        dlist: () => {
          return [
            ...
            },
            {
              mtd: "Q",
              cx:
                this.x3 + this.AX1 + this.NX + this.getARX1() + this.getARX2(),
              cy: this.y1 + this.of - this.A,
...
*/
  ...
  getBleedingLine(type) {
    ...
    let arr = [
      ...
      {
        mtd: "L",
        x: this.x2 - this.AX1 - this.AR + limit * 2,
        y: this.y1 + this.of - this.A,
      },
      {
        mtd: "Q",
